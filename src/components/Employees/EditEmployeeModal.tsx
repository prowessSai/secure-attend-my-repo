import {
  ...
  import { IEmployeeListDTO, ISendInvitationsDTO } from "../../interfaces/User";
  import { editEmployee,} from "../../services/User";
  ...
  const EditEmployeeModal = ({ open, onClose, onSave,employee }: EditEmployeeModalProps) => {
    ...
    useEffect(() => {
      if (open && employee && branches.length > 0 && roles.length > 0) {
        const matchedBranch = branches.find(b => b.branchName === employee.branchName);
        const matchedRole = roles.find(r => r.name === employee.role);
        ...
        setFormData({
          ...
          branchId: matchedBranch?.id || 0,
          roleId: matchedRole?.id || 0,
        });
      }
    }, [open, employee, branches, roles]);
    const handleSend =  async() => {
      ...
      try {
        ...
        const response = await editEmployee(employee.empId, post);
      ...
      <>
        <Modal open={open} onClose={onClose}>
          ...
          >
            ...
            >
              <Typography fontWeight={600} color="#fff">Edit Employee</Typography>
            ...
            >
              <Button variant="outlined" onClick={onClose} sx={{ borderRadius: 20 }}>
                Cancel
              </Button>
