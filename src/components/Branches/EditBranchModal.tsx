...
const EditBranchModal = ({ open, onClose, onSave,branchId }:Props) => {
  ...
  useEffect(() => {
    // ⛔ do not override API prefilled points
    if (isPrefilling || points.length > 0) return;
  ...
  useEffect(() => {
    ...
    const fetchBranch = async () => {
      try {
        ...
        setFenceName(data.branchName || "");
        setBranchAddress(data.branchAddress || "");
  
        const type = data.fenceType?.toLowerCase() as typeof shape;
        setShape(type);
  
        // 🟢 CIRCLE PREFILL
        if (data.fenceType === "CIRCLE") {
          setCenterLat(data.centerLatitude?.toString() || "");
          setCenterLng(data.centerLongitude?.toString() || "");
          setRadius(data.radiusMeters ? (data.radiusMeters - 50).toString() : "");
        }
  
        if (data.fenceType === "RECTANGLE" || data.fenceType === "POLYGON") {
          if (data.coordinates) {
            const coordsString = data.coordinates
              .replace("POLYGON((", "")
              .replace("))", "");
  
            const coordsArray = coordsString.split(",").map((pair: string) => {
              const [lng, lat] = pair.trim().split(" ");
              return { lat, lng };
            });
  
            setPoints(coordsArray.slice(0, -1));
            setSelectedPointIdx(0);
          }
        }
  ...
  // 🔥 sync manual inputs when points are prefilled from API/map
useEffect(() => {
  if (!open) return;
  if (shape === "circle") return;

  if (points.length > 0 && selectedPointIdx === null) {
    setSelectedPointIdx(0);
  }
}, [points, shape, open]);
  ...
  const handleSave = async () => {
    ...
    if (!branchId) {
      setMsg("Invalid branch ID.");
      setOpenError(true);
      return;
    }
  
    const post = {} as IBranchCreateDTO;
    post.branchName = fenceName;
    post.branchAddress = branchAddress;
    post.description = "";
    post.fenceType = shape.toUpperCase() as "CIRCLE" | "RECTANGLE" | "POLYGON";
    
  
    if (shape === "circle") {
      post.centerLatitude = parseFloat(centerLat);
      post.centerLongitude = parseFloat(centerLng);
      post.radiusMeters = parseFloat(radius);
    }
  
    if ((shape === "rectangle" || shape === "polygon") && points.length >= 3) {
      const validPoints = points.filter(p => p.lat && p.lng);
      if (validPoints.length < 3) {
        setMsg("Please fill all points.");
        setOpenError(true);
        return;
      }
      const coords = validPoints.map(p => `${p.lng} ${p.lat}`).join(", ");
      const first = `${validPoints[0].lng} ${validPoints[0].lat}`;
      post.coordinates = `POLYGON((${coords}, ${first}))`;
    }
  
    try {
      setLoading(true);
      const res = await updateBranch(branchId, post);
      console.log("edit response:",res)
      if (res && res.status === 200) {
        setMsg("Geofence updated successfully!");
        setOpenError(false);
        setOpenSuccess(true);
        setTimeout(() => {
      onClose();
    ...
    <Modal open={open} onClose={onClose}>
      <Paper sx={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: "60%", maxWidth: 1000, borderRadius: 2, maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
        <Box sx={{ bgcolor: "#0F4C8199", px: 2, py: 1.5, display: "flex", justifyContent: "space-between", alignItems: "center", borderRadius: "8px 8px 0 0" }}>
          <Typography fontWeight={600} color='#fff'>Edit Branch</Typography>
        ...
        <Box sx={{ p: 2, overflow: "auto", flex: 1 }}>
          <Box sx={{ mb: 2 }}>
            <Typography fontWeight={500} mb={0.5}>
              Geo-Fence Name <span style={{ color: 'red' }}>*</span>
            </Typography>
            <TextField
              value={fenceName}
        ...
        <Box sx={{ display: "flex", justifyContent: "space-between", p: 2, borderTop: "1px solid #E0E0E0" }}>
          <Button onClick={onClose} variant="outlined" sx={{ textTransform: "none", borderRadius: 20, px: 4 }}>Cancel</Button>
          <Button onClick={handleSave} disabled={loading} variant="contained" sx={{ textTransform: "none", borderRadius: 20,bgcolor: "#0F4C81", "&:hover": { bgcolor: "#0D3D66" },px: 4 }}>
            {loading ? (
